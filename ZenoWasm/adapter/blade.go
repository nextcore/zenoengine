...
var (
	...
	VirtualTemplates = make(map[string]string)
	Mu               sync.RWMutex
)

// RegisterTemplate adds a template to the virtual filesystem
func RegisterTemplate(name, content string) {
	Mu.Lock()
	defer Mu.Unlock()
	VirtualTemplates[name] = content
	bladeCache.Delete(name) // Invalidate cache
}
...
func RegisterBladeSlots(eng *engine.Engine) {
	...
	eng.Register("__native_write", func(ctx context.Context, node *engine.Node, scope *engine.Scope) error {
		w, ok := ctx.Value("httpWriter").(io.Writer)
		if !ok {
			return nil
		}
	...
	eng.Register("view.blade", func(ctx context.Context, node *engine.Node, scope *engine.Scope) error {
		...
		// Normalize path
		fullPath := ensureBladeExt(viewFile)
		if !strings.HasPrefix(fullPath, "views/") {
	...
	eng.Register("view.extends", func(ctx context.Context, node *engine.Node, scope *engine.Scope) error {
		...
		fullPath := ensureBladeExt(layoutFile)
		if !strings.HasPrefix(fullPath, "views/") {
			fullPath = "views/" + fullPath
		}
	...
	eng.Register("view.component", func(ctx context.Context, node *engine.Node, scope *engine.Scope) error {
		...
		fullPath := "views/components/" + compPath + ".blade.zl"
...
func parseAndCache(fullPath string) (*engine.Node, error) {
	Mu.RLock()
	content, ok := VirtualTemplates[fullPath]
	Mu.RUnlock()

	if !ok {
		return nil, fmt.Errorf("view not found in virtual fs: %s", fullPath)
	}

	programNode, err := transpileBladeNative(content)
	if err != nil {
		return nil, err
	}
